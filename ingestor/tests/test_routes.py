...
@patch("ingestor.src.routes.PredictorSender.send")
@patch("ingestor.src.routes.OpenSenseFetcher.fetch_latest")
...
@patch(
    "ingestor.src.routes.PredictorSender.send",
...
def test_ingest_route_success(client):
    with (
        ...
        patch(
            "ingestor.src.routes.OpenSenseFetcher.fetch_latest",
            return_value={
                "Temperature": {
                    "value": 22.1,
                    "unit": "°C",
        ...
        patch("ingestor.src.routes.OpenSenseFetcher.__init__", return_value=None),
    ):
        response = client.get("/ingestor/ingest")
        assert response.status_code == 200
        data = response.get_json()
